...
def fetch_current_season():
    ...
    def nc(key):
        ...
        # If all equal (no data), spread by hitter/pitcher count as proxy
        all_eq = len(set(round(v,4) for v in vals)) <= 1
        if all_eq:
            if "Hit" in key or "hit" in key:
                vals = [team_player_groups[tid]["activeHitters"].__len__() +
                        team_player_groups[tid]["benchHitters"].__len__() * 0.5
                        for tid in all_tids]
            else:
                vals = [team_player_groups[tid]["activePitchers"].__len__() +
                        team_player_groups[tid]["benchPitchers"].__len__() * 0.5
                        for tid in all_tids]
