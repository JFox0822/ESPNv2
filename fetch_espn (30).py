...
def main():
    ...
    # If still empty, try week 1 explicitly as last resort
    if not matchups_out:
        print("  ⚠️   matchups_out empty — trying week 1 explicitly")
        try:
            boxes1 = league.box_scores(1)
            for b in boxes1:
                ht, at = b.home_team, b.away_team
                hn = ht.team_name if ht else "BYE"
                an = at.team_name if at else "BYE"
                print(f"  W1 Matchup: {hn} vs {an} (H={b.home_wins} A={b.away_wins})")
                home_raw = get_team_cats(ht, b, is_home=True)
                away_raw = get_team_cats(at, b, is_home=False)
                home_cats, away_cats = resolve_cat_results(home_raw, away_raw)
                home_side = build_side(ht, b.home_wins, b.home_losses, b.home_ties, home_cats, team_map)
            ...
            if matchups_out:
                scoring_week = 1
                print(f"  ✅  Got {len(matchups_out)} matchups from week 1 fallback")
        except Exception as e2:
            print(f"  ⚠️   Week 1 fallback also failed: {e2}")
