...
def main():
    ...
    # ── Matchups via direct ESPN API ─────────────────────────────────────────
    # espn-api's box_scores() doesn't reliably return H2H category data for baseball.
    # We call ESPN's REST API directly using the same cookies.
    matchups_out = []
    try:
        ...
        # Try scoring periods 1 and 2 to find which has active matchups
        for sp in [scoring_week, max(1, scoring_week - 1)]:
            url = (f"https://fantasy.espn.com/apis/v3/games/flb/seasons/{SEASON}"
                   f"/segments/0/leagues/{LEAGUE_ID}"
                   f"?view=mMatchup&view=mMatchupScore&scoringPeriodId={sp}")
            resp = req_lib.get(url, cookies=cookies, timeout=15)
            print(f"  ESPN API sp={sp}: HTTP {resp.status_code}")
            if resp.status_code != 200:
                continue

            raw = resp.json()
            schedule = raw.get("schedule", [])
            # Filter to matchups in this scoring period
            period_matches = [m for m in schedule
                              if m.get("matchupPeriodId") == sp or
                                 m.get("home", {}).get("currentPeriodRealStats") or
                                 m.get("home", {}).get("totalPoints") is not None]
            if not period_matches:
                period_matches = [m for m in schedule if m.get("matchupPeriodId") == sp]
            print(f"  Found {len(period_matches)} matchups for sp={sp}")
            ...
            def parse_matchup_side(side_data):
                """Parse one side of a matchup from ESPN raw API."""
                tid = side_data.get("teamId")
                tname = id_to_name.get(tid, f"Team {tid}")
                tm = team_map.get(tid, {})

                # Get cumulative stats for the scoring period
                stats = {}
                for src_key in ["currentPeriodRealStats","totalPointsLive","cumulativeScore"]:
                    src = side_data.get(src_key, {})
                    if isinstance(src, dict):
                        score_by_stat = src.get("scoreByStat", {})
                        if score_by_stat:
                            for stat_id, info in score_by_stat.items():
                ...
                cat_wins = side_data.get("totalPoints", 0) or 0
                cat_losses = side_data.get("totalPointsLive", 0) or 0
                # In H2H categories, ESPN uses "wins" field differently
                # Try to get from cumulativeScore
                cum = side_data.get("cumulativeScore", {})
                if cum:
                    cat_wins   = cum.get("wins", cat_wins) or cat_wins
                    cat_losses = cum.get("losses", cat_losses) or cat_losses
                ...
                return {
                    ...
                    "catWins": int(cat_wins),
                    "catLoss": int(cat_losses),
                    "catTies": 0,
                    "stats":   stats,
                }
            ...
            for m in period_matches:
                ...
                if not home_data.get("teamId") or not away_data.get("teamId"):
                    continue

                home_side = parse_matchup_side(home_data)
                away_side = parse_matchup_side(away_data)

                # Compute category win/loss for each stat
                home_cats, away_cats = {}, {}
                all_lbls = set(list(home_side["stats"].keys()) + list(away_side["stats"].keys()))
                for lbl in all_lbls:
                    hv = home_side["stats"].get(lbl)
                    av = away_side["stats"].get(lbl)
                    if hv is None or av is None:
                        continue
                    lower = lbl in LOWER
                    fmt = lambda v, l: (f"{v:.3f}" if l in {"AVG","OPS"} else
                                        f"{v:.2f}" if l in {"ERA","WHIP"} else
                                        f"{v:.1f}" if l == "IP" else str(int(v)) if v==int(v) else str(round(v,1)))
                    if abs(hv - av) < 0.0001:
                        h_res = a_res = "TIE"
                    elif (hv < av) == lower:
                        h_res, a_res = "WIN", "LOSS"
                        ...
                        h_res, a_res = "LOSS", "WIN"
                    home_cats[lbl] = {"value": fmt(hv, lbl), "result": h_res}
                    away_cats[lbl] = {"value": fmt(av, lbl), "result": a_res}

                home_side["categories"] = home_cats
                away_side["categories"] = away_cats
                del home_side["stats"]
                del away_side["stats"]
                ...
                print(f"    {home_side['team']} ({home_side['catWins']}) vs "
                      f"{away_side['team']} ({away_side['catWins']})")
        ...
        import traceback
        print(f"  ⚠️   Matchup fetch failed: {e}")
        traceback.print_exc()
    save("matchups.json", {
        "week":     scoring_week,
