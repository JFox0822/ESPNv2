...
def fetch(*views):
    ...
    try:
        ...
        print(f"  → HTTP {r.status_code} ({r.headers.get('Content-Type','?')[:50]})")
        ...
        if "application/json" not in ct and "text/plain" not in ct:
            print(f"❌  ESPN returned non-JSON. Preview: {r.text[:300]}", file=sys.stderr)
            sys.exit(1)
        return r.json()
        ...
        print(f"❌  JSON decode error: {e}. Preview: {r.text[:300]}", file=sys.stderr)
        sys.exit(1)
