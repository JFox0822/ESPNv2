...
def main():
    ...
    try:
        ...
        def api_get(views, extra_params=None):
            """Call espn-api's internal request handler."""
            if isinstance(views, str):
                views = [views]
            params = {'view': views}
            if extra_params:
                params.update(extra_params)
            # Try keyword arg style first
            try:
                return req.get('', **{k: v for k, v in params.items()})
            except TypeError:
                pass
            # Try dict style
            try:
                ...
                pass
            # Try the underlying session directly
            for attr in ['session', '_session', 'r']:
                s = getattr(req, attr, None)
                if s and hasattr(s, 'get'):
                    base = getattr(req, 'FANTASY_BASE_ENDPOINT',
                                   getattr(req, 'base_url', ''))
                    if not base:
                        base = (f'https://lm-api-reads.fantasy.espn.com/apis/v3'
                                f'/games/flb/seasons/{SEASON}/segments/0'
                                f'/leagues/{LEAGUE_ID}')
                    resp = s.get(base, params=params)
                    return resp.json() if hasattr(resp, 'json') else resp
            raise RuntimeError("No working request method found")
