    ...
    try:
        ...
        for sp in [1, 2]:
            ...
            if hasattr(session, '__call__'):
                # It's a callable (league.espn_request is a function)
                try:
                    raw = session('GET', url)
                    print(f"  ESPN lm-api sp={sp}: via callable")
                except Exception as e:
                    print(f"  ⚠️  callable failed: {e}")
            
            if raw is None:
                # It's a requests.Session
                try:
                    resp = session.get(url, timeout=15)
                    print(f"  ESPN lm-api sp={sp}: HTTP {resp.status_code}")
                    if resp.status_code == 200:
                        raw = resp.json()
                    else:
                        # Try old endpoint
                        url2 = (f"https://fantasy.espn.com/apis/v3/games/flb"
                                f"/seasons/{SEASON}/segments/0/leagues/{LEAGUE_ID}"
                                f"?view=mMatchup&view=mMatchupScore&scoringPeriodId={sp}")
                        resp2 = session.get(url2, timeout=15)
                        print(f"  ESPN fantasy sp={sp}: HTTP {resp2.status_code}")
