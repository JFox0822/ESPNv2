...
def main():
    ...
    # ── Matchups via ESPN API using espn-api's authenticated session ──────────
    matchups_out = []
    try:
        # espn-api's League object has an internal session with working cookies/headers
        # Access it via league.espn_request (the authenticated requests function)
        # This avoids the BoxScore abstract class bug and the 403 from raw requests

        # Get the internal session from espn-api
        session = None
        for attr in ['espn_request', '_session', 'session', 'requests_session']:
            s = getattr(league, attr, None)
            if s is not None:
                session = s
                print(f"  📡  Using league.{attr} for API call")
                break

        if session is None:
            # Build a session manually using espn-api's cookie format
            import requests as req_lib
            session = req_lib.Session()
            session.cookies.set("espn_s2", ESPN_S2)
            session.cookies.set("SWID", ESPN_SWID)
            session.headers.update({
                ...
                "X-Fantasy-Platform": "kona-PROD-c40986dc1b8ab4dfd5de5d74b6e641a3387a0b4c",
            })
            print("  📡  Built manual session with cookies")

        for sp in [1, 2]:
            url = (f"https://lm-api-reads.fantasy.espn.com/apis/v3/games/flb"
                   f"/seasons/{SEASON}/segments/0/leagues/{LEAGUE_ID}"
                   f"?view=mMatchup&view=mMatchupScore&scoringPeriodId={sp}")

            # Try espn-api's own request method first
            raw = None
            # EspnFantasyRequests.get() takes (url, params=None) - no timeout
            # It returns a dict (already parsed JSON) not a Response object
            try:
                ...
                for k, vs in parse_qs(parsed.query).items():
                    params[k] = vs  # keep as list for multi-value params like view
                base_url = f"{parsed.scheme}://{parsed.netloc}{parsed.path}"
                
                result = session.get(base_url, params=params)
                print(f"  ESPN lm-api sp={sp}: got result type={type(result).__name__}")
                ...
                # Last resort: rebuild with requests using espn-api's cookie jar
                try:
                    import requests as req_lib
                    s2 = req_lib.Session()
                    s2.cookies.set("espn_s2", ESPN_S2)
                    s2.cookies.set("SWID", ESPN_SWID)
                    s2.headers.update({
                    ...
                    print(f"  fallback requests sp={sp}: HTTP {r.status_code}")
            ...
            for m in schedule:
                ...
                def parse_side(side):
                    ...
                    # Try cumulativeScore.scoreByStat (primary for H2H categories)
                    ...
                    cat_wins   = int(cum.get("wins",   side.get("totalPoints",   0)) or 0)
                    cat_losses = int(cum.get("losses", side.get("totalPointsLive",0)) or 0)
                ...
                home_s = parse_side(home_d)
                away_s = parse_side(away_d)

                # Category win/loss per stat
                home_cats, away_cats = {}, {}
                for lbl in set(list(home_s["stats"]) + list(away_s["stats"])):
                    hv = home_s["stats"].get(lbl)
                    av = away_s["stats"].get(lbl)
                    if hv is None or av is None: continue
                    lower = lbl in LOWER
                    if abs(hv - av) < 0.0001: h_r = a_r = "TIE"
                    elif (hv < av) == lower:  h_r, a_r = "WIN", "LOSS"
                    else:                      h_r, a_r = "LOSS", "WIN"
                    home_cats[lbl] = {"value": fmt_v(hv,lbl), "result": h_r}
                    away_cats[lbl] = {"value": fmt_v(av,lbl), "result": a_r}

                home_s["categories"] = home_cats; del home_s["stats"]
                away_s["categories"] = away_cats; del away_s["stats"]

                hw = home_s["catWins"] > away_s["catWins"]
                aw = away_s["catWins"] > home_s["catWins"]
                matchups_out.append({
                    "home":   home_s, "away": away_s,
                    "leader": home_s["team"] if hw else (away_s["team"] if aw else "Tied"),
                    "winner": "UNDECIDED",
                })
                print(f"    ✓ {home_s['team']} ({home_s['catWins']}) vs {away_s['team']} ({away_s['catWins']})")
