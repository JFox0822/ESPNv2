...
TOTAL_TEAMS     = 12
PLAYOFF_SPOTS   = 6
FULL_ACTUAL_WEEK = 12   # by week 12, use 100% actuals; fade from 90% proj at week 1
...
def proj_weight(week):
    """90% proj at week 1, linearly fades to 0% at FULL_ACTUAL_WEEK."""
    ...
    try:
        for bs in league.box_scores(week):
            def side(team, sc):
                ...
                cw = getattr(sc,"wins",0) if sc else 0
                cl = getattr(sc,"losses",0) if sc else 0
                return {
                    "teamId":  team.team_id,
                    "team":    team.team_name,
                    "abbrev":  team.team_abbrev,
                    "catWins": cw, "catLoss": cl,
                    "categories": {},
            ...
            winner = getattr(bs,"winner","UNDECIDED")
